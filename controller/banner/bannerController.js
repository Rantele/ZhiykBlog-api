 ...
 * @LastEditors: Rantele
 * @LastEditTime: 2022-10-28 18:57:21
 * @Description:
 *
 */
    ...
    if (err) {
      ...
      setImage(nowDate, bannerType, "banner", files.bannerfile.filepath)
        .then(() => {
          //上传成功
          return query(
            "INSERT INTO banners (location,img,description,createTime) VALUES (?,?,?,?)",
            ["home_banner", bannerName, "首页轮播图", create_time]
          );
        })
        .then(() => {
          res.send({
...
deleteBanner = (req, res) => {
  ...
  if (!id || !img) {
    ...
    query(
      "select id from banners where id=? and location='home_banner' and img=?",
      [id, img]
    )
      .then((result) => {
        if (result.length === 0) {
        ...
        return query(
          "delete from banners where id=? and location='home_banner' and img=?",
          [id, img]
        );
      })
      .then(() => {
