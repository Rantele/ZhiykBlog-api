      ...
      setImage(nowDate, iconType, 'websites', files.iconfile.filepath)
        .then(() => {
          ...
          return query(
            ...
            [Number.parseInt(parentId), title, name, description || null, link || null, icon, create_time]
          )
        })
        .then(() => {
          res.send({
  ...
  if (parentId === 0) {
    ...
    query('select id from websites where parentId=?', [id])
      .then((result) => {
        ...
        return query('delete from websites where id=? and parentId=?', [id, parentId])
      })
      .then(() => {
        res.send({
          code: 200,
...
updateWS = (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    if (err) {
      ...
      //判断是否修改图片
      let sql = 'update websites set parentId=?,title=?,name=?,description=?,link=? where id=?'
      let sqlArr = [parseInt(parentId), title, name, description, link, parseInt(id)]
      ...
      if (Object.keys(files).length !== 0) {
        ...
        sql = 'update websites set parentId=?,title=?,name=?,description=?,link=?,icon=? where id=?'
        sqlArr = [parseInt(parentId), title, name, description, link, newIcon, parseInt(id)]
        await setImage(nowDate, iconType, 'websites', files.iconfile.filepath).catch((err) => {
          console.log('[catch]:', err)
          res.status(500).send({
            code: -1,
            message: '服务器错误',
